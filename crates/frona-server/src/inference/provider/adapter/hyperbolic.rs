...
pub(crate) fn build(
    ...
    let endpoint = connection
        .effective_base_url
        .as_deref()
        .expect("Hyperbolic endpoint");
    let client = hyperbolic::Client::builder()
        .api_key(&key)
        .base_url(endpoint)
        .http_client(WireClient::default())
        .build()
        .map_err(|_| InferenceError::ConfigError("Cannot build Hyperbolic client".into()))?;
    let discovery = openai::CompletionsClient::builder()
        ...
        .build()
        .map_err(|_| {
            InferenceError::ConfigError("Cannot build Hyperbolic model discovery client".into())
        })?;
    Ok(Arc::new(
...
mod tests {
    ...
    async fn live_directory_uses_v1_models_and_preserves_empty_inventory() {
        ...
        let provider = provider("hyperbolic", &format!("{}/proxy", server.uri()));
        for body in [
