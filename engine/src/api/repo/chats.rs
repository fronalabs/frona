...
impl ChatRepository for SurrealRepo<Chat> {
    async fn find_by_user_id(&self, user_id: &str) -> Result<Vec<Chat>, AppError> {
        let query =
            format!("{SELECT_CLAUSE} FROM chat WHERE user_id = $user_id ORDER BY updated_at DESC");
        let mut result = self
            .db()
            .query(&query)
    ...
    async fn find_standalone_by_user_id(&self, user_id: &str) -> Result<Vec<Chat>, AppError> {
        let query = format!(
            "{SELECT_CLAUSE} FROM chat WHERE user_id = $user_id AND space_id IS NONE ORDER BY updated_at DESC"
        );
        let mut result = self
            .db()
            .query(&query)
